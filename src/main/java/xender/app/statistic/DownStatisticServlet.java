...
public class DownStatisticServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		if (code != null) {
			...
			String[] dates = StatisticUT.getStartEnd(code, "app", start, end);
			start = dates[0];
			end = dates[1];
			
			String sql = "select code,DATE_FORMAT(time,'%Y/%m/%d') as time ,count from downstatistic  " + "where code='" + code + "' and  time<='" + end + "' and time>='" + start + "'  order by time asc";
			...
			try {
				...
				if (Lang.isEmpty(array)) {
					array = StatisticUT.get7Default(code,"app",start,end);
				}
