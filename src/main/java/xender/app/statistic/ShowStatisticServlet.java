...
public class ShowStatisticServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		if (start == null || end == null) {
			start = (String) Util.retrieveObject(request, "show_start");
			end = (String) Util.retrieveObject(request, "show_end");
		}
		...
		if (start == null || "".equals(start) || end == null || "".equals(end)) {
			...
			Util.putSession(request, "show_start", start);
			Util.putSession(request, "show_end", end);
		}
		...
		if (code != null) {
			...
			String sql = "select code,DATE_FORMAT(time,'%Y/%m/%d') as time ,count from showstatistic  " + "where code='" + code + "' and  time<='" + end + "' and time>='" + start + "'  order by time asc";
